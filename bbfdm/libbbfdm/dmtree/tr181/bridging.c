...
static void add_wireless_port_to_bridge_sections(struct uci_section *br_sec, struct uci_section *br_dmmap_sec, char *device_port)
{
	struct uci_list *uci_list = NULL;
	char *br_name = NULL, *network = NULL, *new_network = NULL;

	if (DM_STRLEN(device_port) == 0)
		return;

	if (br_sec) {
		dmuci_get_value_by_section_string(br_sec, "name", &br_name);
		struct uci_section *if_sec = get_dup_section_in_config_opt("network", "interface", "device", br_name);
		struct uci_section *wifi_sec = get_origin_section_from_config("wireless", "wifi-iface", device_port);
		if ((wifi_sec) && (if_sec)){
			
			dmuci_get_value_by_section_string(wifi_sec, "network", &network);
			new_network = add_str_to_str_list(network, " ", section_name(if_sec));
			dmuci_set_value_by_section(wifi_sec, "network", new_network);
		}	
	}
...
static void remove_wireless_port_from_bridge_sections(struct uci_section *br_sec, struct uci_section *br_dmmap_sec, char *device_port)
{
	struct uci_list *uci_list = NULL;
	char *br_name = NULL, *network = NULL, *new_network = NULL;
	...
	if (br_sec) {
		...
		if ((wifi_sec) && (if_sec)){
			...
			dmuci_get_value_by_section_string(wifi_sec, "network", &network);
			new_network = remove_str_from_str_list(network, " ", section_name(if_sec));
			dmuci_set_value_by_section(wifi_sec, "network", new_network);
		}			
	}
...
{
	struct uci_section *s = NULL, *dmmap_sect = NULL, *stmp = NULL;
	char *br_name = NULL;

	// ADDED IN PFE ( DELETED DUPLICATED PORTS)
	//eliminate_duplicates_in_portlist();
	...
	uci_foreach_option_eq(package, section_type, option_name, option_value, s) {
		...
		dmuci_get_value_by_section_string(s, "name", &br_name);
		if ((dmmap_sect = get_dup_section_in_dmmap_opt(dmmap_package, section_type, "name", br_name)) == NULL) {
			struct uci_list *ports_list = NULL;

			dmuci_add_section_bbfdm(dmmap_package, "device", &dmmap_sect);
			dmuci_set_value_by_section(dmmap_sect, "name", br_name);
			...
			// ADDED IN PFE 
			//refresh ports
			struct uci_list *conf_ports_list = NULL, *dmmap_ports_list = NULL;

			dmuci_get_value_by_section_list(s, "ports", &conf_ports_list);
			dmuci_get_value_by_section_list(dmmap_sect, "ports", &dmmap_ports_list);
			if (conf_ports_list != NULL) {
				struct uci_element *e = NULL;

				uci_foreach_element(conf_ports_list, e) {
					if (!value_exists_in_uci_list(dmmap_ports_list, e->name))
						dmuci_add_list_value_by_section(dmmap_sect, "ports", e->name);
				}
			}
		}
	...
	uci_path_foreach_sections_safe(bbfdm, dmmap_package, section_type, stmp, s) {
		dmuci_get_value_by_section_string(s, "name", &br_name);
		if (get_dup_section_in_config_opt(package, "device", "name", br_name) == NULL)
			dmuci_delete_by_section(s, NULL, NULL);
	...
	uci_path_foreach_sections(bbfdm, dmmap_package, "device", dmmap_sect) {
		struct uci_section *origin_s = NULL;
		char *name = NULL, *config = NULL;

		dmuci_get_value_by_section_string(dmmap_sect, "name", &name);
		dmuci_get_value_by_section_string(dmmap_sect, "config", &config);

		origin_s = get_dup_section_in_config_opt(package, section_type, "name", name);
...
static void dmmap_synchronizeBridgingBridgeVLAN(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	struct uci_section *s = NULL, *stmp = NULL, *s_br = NULL;
	...
	uci_path_foreach_option_eq_safe(bbfdm, "dmmap_bridge_vlan", "bridge_vlan", "br_inst", args->br_inst, stmp, s) {
		...
		uci_foreach_element(br_ports_list, e) {
			if (get_dup_section_in_config_opt("network", "bridge-vlan", "vlan", vid)) {
				vid_found = true;
				break;
			}
		}
	...
	uci_foreach_sections("network", "bridge-vlan", s) {
		struct uci_section *br_vlan_s = NULL;
		char *vid = NULL, *device = NULL, *br_inst = NULL;
		char vlan_name[32]= {0};

		dmuci_get_value_by_section_string(s, "device", &device);

		s_br = get_dup_section_in_dmmap_opt("dmmap_bridge_port", "bridge_port", "port", device);
		dmuci_get_value_by_section_string(s_br, "br_inst", &br_inst);
			
		if(DM_LSTRCMP(br_inst, args->br_inst) == 0){
	
			dmuci_get_value_by_section_string(s, "vlan", &vid);
			...
			snprintf(vlan_name, sizeof(vlan_name), "br_%s_vlan_%s", args->br_inst, vid);
...
//ADDED IN PFE
static int get_br_port_list_from_conf(struct uci_list *ports_list, char port_buf[128])
{
	struct uci_element *e = NULL;
	char port[16] = {'\0'};
	...
	uci_foreach_element(ports_list, e) {
		...
		if (strstr(e->name,":t")){
			char *p = strstr(e->name,":t");
			strncpy(port,e->name,(int)(p-(e->name)));
			port[strlen(port)] = ' ';
		}else{
			strncpy(port, e->name, strlen(e->name));
		}
		strncat( port_buf, port, strlen(port));
	}
	port_buf[strlen(port_buf)] = '\0';
	return 0;
}

//ADDED IN PFE
static struct uci_section *get_origin_vlanport_section_from_conf(char *package, char *section_type, char *vid, char *port)
{
	struct uci_section *s = NULL;
	struct uci_list *ports_list = NULL;

	uci_foreach_option_eq(package, section_type, "vlan", vid, s) {
		
		char ports_list_buf[128] = {'\0'};

		dmuci_get_value_by_section_list(s, "ports", &ports_list);
		if (ports_list == NULL)
			continue;
		
		get_br_port_list_from_conf(ports_list,ports_list_buf);
		if ((DM_STRSTR(ports_list_buf, port)))
			return s;
	}
...
static void dmmap_synchronizeBridgingBridgeVLANPort(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	uci_path_foreach_option_eq_safe(bbfdm, "dmmap_bridge_vlanport", "bridge_vlanport", "br_inst", args->br_inst, stmp, s) {
		...
		char *s_user = NULL;
		dmuci_get_value_by_section_string(s, "added_by_user", &s_user);
		if ((DM_STRLEN(vid)==0) && s_user && DM_LSTRCMP(s_user, "1") == 0)
			continue;
		...
		uci_foreach_element(br_ports_list, e) {
			...
			if (DM_STRSTR(e->name, port)) {
				if (get_origin_vlanport_section_from_conf("network", "bridge-vlan", vid, port)){
					port_found = true;
					break;
				}
			}
	...
	uci_foreach_element(br_ports_list, e) {
		struct uci_list *ports_list = NULL;
		char *lower_layer = NULL, *device = NULL, *br_inst = NULL, *vid = NULL;
		...
		uci_foreach_sections("network", "bridge-vlan", s) {
			...
			s_br = get_dup_section_in_dmmap_opt("dmmap_bridge_port", "bridge_port", "port", device);
			dmuci_get_value_by_section_string(s_br, "br_inst", &br_inst);
			int vlan_found = dmuci_get_value_by_section_string(s, "vlan", &vid);

			if((vlan_found == 0) && (DM_STRNCMP(br_inst, args->br_inst,strlen(br_inst)) == 0)){

				if(get_section_in_dmmap_with_options_eq("dmmap_bridge_vlanport", "bridge_vlanport", "br_inst", args->br_inst, "vid", vid))
					continue;
				...
				dmuci_set_value_by_section(br_vlanport_s, "vid", vid);
				
				get_br_port_list_from_conf(ports_list,ports_list_buf);
				if (DM_STRSTR(ports_list_buf, e->name))
					dmuci_set_value_by_section(br_vlanport_s, "Port", e->name);
...
static bool is_wireless_port_in_bridge(struct uci_section *ifsec, char *br_name)
{
	struct uci_section *l3_sec = NULL;
  	char * l3_name = NULL, *l3_device = NULL;
	bool found = false;
	char *network_op = NULL;

	dmuci_get_value_by_section_string(ifsec,"network",&network_op);

  	l3_name = strtok (network_op," ");
  	while (l3_name != NULL)
  	{
    	l3_sec = get_origin_section_from_config("network", "interface", l3_name);
		if (l3_sec){
			dmuci_get_value_by_section_string(l3_sec, "device", &l3_device);
			if (DM_STRCMP(l3_device, br_name) == 0){
				found = true;
				break;
			}
		}
...
/*static int get_last_instance_bridge_port(char *package, char *section, char *opt_inst)
{
	struct uci_section *s;
	int inst = 0;
...
static void dmmap_synchronizeBridgingBridgePort(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	struct uci_section *s = NULL, *stmp = NULL;
	struct uci_element *e = NULL;
	char *s_user = NULL, *type = NULL;


	if (!args->bridge_sec)
		return;

	// skip option different of bridge
	dmuci_get_value_by_section_string(args->bridge_sec, "type", &type);
	...
	if (!get_section_in_dmmap_with_options_eq("dmmap_bridge_port", "bridge_port", "br_inst", args->br_inst, "management", "1"))
	...
	if (interface_s){
		...
		uci_foreach_sections("wireless", "wifi-iface", s) {
			...
			if (!strstr(network,section_name(interface_s)))
				continue;
		
			if (get_section_in_dmmap_with_options_eq("dmmap_bridge_port", "bridge_port", "br_inst", args->br_inst, "port", section_name(s)))
				continue;

			create_new_bridge_port_section("wireless", section_name(s), args->br_inst, "0");
			dmuci_add_list_value_by_section(args->bridge_dmmap_sec, "ports", section_name(s));
		}
	}
	...
	uci_path_foreach_sections_safe(bbfdm, "dmmap_bridge_port", "bridge_port", stmp, s) {
		...
		//ADDED IN PFE
		// section for delete 
		char *management = NULL, *port_name = NULL;
		
		dmuci_get_value_by_section_string(s, "management", &management);
		dmuci_get_value_by_section_string(s, "port", &port_name);
		
		if ((DM_LSTRCMP(management, "1") == 0) && (get_dup_section_in_config_opt("network", "device", "name", port_name) == NULL))
			dmuci_delete_by_section(s, NULL, NULL);
		...
		if (DM_LSTRCMP(management, "0") == 0) {

			struct uci_section *br_dmmap_sec = NULL, *br_conf_sec = NULL;
			struct uci_list *ports_list = NULL;
			char *br_inst = NULL, *br_name = NULL, *enabled = NULL;

			dmuci_get_value_by_section_string(s, "br_inst", &br_inst);
			dmuci_get_value_by_section_string(s, "port", &port_name);
			...
			br_dmmap_sec = get_dup_section_in_dmmap_opt("dmmap_bridge", "device", "bridge_instance", br_inst);
			
			if (br_dmmap_sec == NULL){
				dmuci_delete_by_section(s, NULL, NULL);
				continue;
			}
			
			dmuci_get_value_by_section_string(br_dmmap_sec, "name", &br_name);
			br_conf_sec = get_dup_section_in_config_opt("network", "device", "name", br_name);

			//wifi bridge
			struct uci_section *ifsec = get_origin_section_from_config("wireless", "wifi-iface", port_name);
			if (ifsec){
				
				if (is_wireless_port_in_bridge(ifsec,br_name)){
					continue;
				}else{
					dmuci_delete_by_section(s, NULL, NULL);
					dmuci_del_list_value_by_section(br_dmmap_sec, "ports", port_name);
					continue;
				}
					
			}

			dmuci_get_value_by_section_list(br_conf_sec, "ports", &ports_list);
			...
			if (!value_exists_in_uci_list(ports_list, port_name)){
				dmuci_delete_by_section(s, NULL, NULL);
				dmuci_del_list_value_by_section(br_dmmap_sec, "ports", port_name);
			}
...
//MODIFIED IN PFE
static void get_bridge_vlan_config_section(struct uci_section *bridge_vlanport_dmmap_sec, struct uci_section **device_section)
{
	struct uci_section *s = NULL;
	char *vlan = NULL, *br_inst = NULL, *br_name = NULL;
	*device_section = NULL;
	...
	dmuci_get_value_by_section_string(bridge_vlanport_dmmap_sec, "vid", &vlan);
	if (DM_STRLEN(vlan) == 0)
		return;

	dmuci_get_value_by_section_string(bridge_vlanport_dmmap_sec, "br_inst", &br_inst);
	struct uci_section *br_sec = get_dup_section_in_dmmap_opt("dmmap_bridge", "device", "bridge_instance", br_inst);
	if (br_sec == NULL)
		return;
	dmuci_get_value_by_section_string(br_sec, "name", &br_name);

	/* Find the device network section corresponding to this device_name */
	uci_foreach_option_eq("network", "bridge-vlan", "vlan", vlan, s) {
		char *device = NULL;
		dmuci_get_value_by_section_string(s, "device", &device);
		if (DM_STRCMP(device, br_name) == 0){
			*device_section = s;
			return;
		}
	}
...
static void get_bridge_port_device_section(struct uci_section *bridge_port_dmmap_sec, struct uci_section **device_section)
{
	...
	*device_section = NULL;
	...
	uci_foreach_sections("wireless", "wifi-iface", s) {
		if (strcmp(section_name(s), port) == 0){
			*device_section = s;
			return;
		}
...
static void Update_BridgeVLANPort_Port_Layer(char *path, struct uci_section *bridge_sec, struct uci_section *bridge_dmmap_sec, char *bridge_intsance, char *linker)
{
	...
	uci_path_foreach_option_eq(bbfdm, "dmmap_bridge_vlanport", "bridge_vlanport", "br_inst", bridge_intsance, dmmap_s) {
		...
		device_s = get_origin_section_from_config("network", "bridge-vlan", sec_name);
		if (!device_s)
			return;

		dmuci_get_value_by_section_string(device_s, "vlan", &vlan);

		snprintf(new_name, sizeof(new_name), "%s%s%s", DM_STRLEN(linker) ? linker : "",
								(DM_STRLEN(linker) && DM_STRLEN(vlan)) ? "." : "",
								(DM_STRLEN(linker) && DM_STRLEN(vlan)) ? vlan : "");
...
/*static void Update_BridgeVLANPort_VLAN_Layer(char *path, struct uci_section *bridge_sec, struct uci_section *bridge_dmmap_sec, char *bridge_intsance, char *linker)
{
	...
	uci_path_foreach_option_eq(bbfdm, "dmmap_bridge_vlanport", "bridge_vlanport", "br_inst", bridge_intsance, dmmap_s) {
		...
		dmuci_set_value_by_section(device_s, "vlan", linker);
...
static int browseBridgingBridgeVLANPortInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	uci_path_foreach_option_eq(bbfdm, "dmmap_bridge_vlanport", "bridge_vlanport", "br_inst", br_args->br_inst, br_vlanport_dmmap_s) {
		...
		get_bridge_vlan_config_section(br_vlanport_dmmap_s, &br_vlanport_s);
...
static int delObjBridgingBridgePort(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
	...
	case DEL_INST:
		if (!args->is_management_port) {
			...
			if (DM_STRCMP(config, "network") == 0) {
				remove_port_from_bridge_sections(args->bridge_sec, args->bridge_dmmap_sec, port);
			}else{
				char *network = NULL, *br_name = NULL, *new_network = NULL;
				dmuci_get_value_by_section_string(args->bridge_port_sec, "network", &network);
				dmuci_get_value_by_section_string(args->bridge_sec, "name", &br_name);
				
				struct uci_section *if_sec = get_dup_section_in_config_opt("network", "interface", "device", br_name);
				if (strstr(network,section_name(if_sec))){
					new_network = remove_str_from_str_list(network, " ", section_name(if_sec));
					dmuci_set_value_by_section(args->bridge_port_sec, "network", new_network);
					dmuci_del_list_value_by_section(args->bridge_dmmap_sec, "ports", port);
					dmuci_commit_package("wireless");
...
static int addObjBridgingBridgeVLANPort(char *refparam, struct dmctx *ctx, void *data, char **instance)
{
	...
	// Add dmmap section
	dmuci_add_section_bbfdm("dmmap_bridge_vlanport", "bridge_vlanport", &dmmap_sec);
	dmuci_set_value_by_section(dmmap_sec, "br_inst", ((struct bridge_args *)data)->br_inst);
	dmuci_set_value_by_section(dmmap_sec, "bridge_vlanport_instance", *instance);
	dmuci_set_value_by_section(dmmap_sec, "added_by_user", "1");
...
static int delObjBridgingBridgeVLANPort(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
	...
	struct uci_list *port_list = NULL;
	char *port_ref = NULL, *port_buf = NULL;
	...
	case DEL_INST:
		...
		dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "Port", &port_ref);
		dmasprintf(&port_buf, "%s:t", port_ref);
		dmuci_get_value_by_section_list(args->bridge_vlanport_sec, "ports", &port_list);

		if (value_exists_in_uci_list(port_list, port_buf))
			dmuci_del_list_value_by_section(args->bridge_vlanport_sec, "ports", port_buf);

		else if (value_exists_in_uci_list(port_list, port_ref))
			dmuci_del_list_value_by_section(args->bridge_vlanport_sec, "ports", port_ref);
...
{
	struct uci_section *br_vlan_s = NULL, *br_vlan_conf = NULL;
	char vlan_name[32]= {0};
	char vid_str[32] = {0};
	char *br_name = NULL;
	...
	dmuci_get_value_by_section_string(((struct bridge_args *)data)->bridge_sec,"name",&br_name);
	dmuci_add_section("network", "bridge-vlan", &br_vlan_conf);
	dmuci_set_value_by_section(br_vlan_conf, "device", br_name);
	dmuci_set_value_by_section(br_vlan_conf, "vlan", vid_str);
...
static int delObjBridgingBridgeVLAN(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
	...
	struct uci_section *br_dmmap_sec = NULL, *br_vlan_sec = NULL;
	char *vid = NULL, *br_inst = NULL, *br_name = NULL, *vlan_op = NULL;
	...
	case DEL_INST:
		...
		dmuci_get_value_by_section_string(args->bridge_vlan_sec, "br_inst", &br_inst);
		br_dmmap_sec = get_dup_section_in_dmmap_opt("dmmap_bridge", "device", "bridge_instance",br_inst);
		if (br_dmmap_sec){
			dmuci_get_value_by_section_string(br_dmmap_sec, "name", &br_name);
			br_vlan_sec = get_dup_section_in_config_opt("network", "bridge-vlan", "device", br_name);
		}
		if (br_vlan_sec){
			dmuci_get_value_by_section_string(br_vlan_sec, "vlan", &vlan_op);
			if (DM_STRCMP(vlan_op, vid) == 0)
				dmuci_delete_by_section(br_vlan_sec, NULL, NULL);
		}
...
{
	struct bridge_port_args *args = (struct bridge_port_args *)data;
	char *config = NULL, *port = NULL, *br_name = NULL;
	bool b;
	...
	switch (action) {
		...
		case VALUESET:
			...
			if (DM_LSTRCMP(config, "wireless") == 0) {
				struct uci_section *wifi_iface_s = get_dup_section_in_config_opt("network", "interface", "device", br_name);
		...
		*value = dmstrdup(buf);
		...
		if ((*value)[0] == '\0') {
			...
			if (DM_LSTRCMP(config, "network") == 0) {
				...
				adm_entry_get_reference_param(ctx, "Device.WiFi.SSID.*.Name", port, value);
			}
...
static int set_BridgingBridgePort_LowerLayers(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	char *enable = NULL, *port = NULL, *type = NULL, *LowerLayers = NULL;
	...
	switch (action) {
		case VALUECHECK:
			...
			//eliminate dup
			if (get_section_in_dmmap_with_options_eq("dmmap_bridge_port", "bridge_port", "LowerLayers", reference.path,"management","0"))
				return FAULT_9007;
		...
		case VALUESET:
			...
			if ((DM_STRCMP(enable, "1") == 0) && (DM_STRLEN(port)!=0)) {

				dmuci_get_value_by_section_string(args->bridge_port_dmmap_sec, "LowerLayers", &LowerLayers);
				
				if (DM_LSTRNCMP(LowerLayers, "Device.WiFi.SSID.", 17) == 0){
					remove_wireless_port_from_bridge_sections(args->bridge_sec, args->bridge_dmmap_sec, port);
				}else if (DM_LSTRNCMP(LowerLayers, "Device.Ethernet.Interface.", 26) == 0){
			...
			struct uci_section *port_s = NULL;
			char *inst = NULL, *port_name = NULL;

			if (DM_LSTRNCMP(reference.path, "Device.Ethernet.Interface.", 26) == 0){
				inst = strrchr(reference.path, '.');
				port_s = get_dup_section_in_dmmap_opt("dmmap_ethernet", "interface", "ethernet_iface_instance", inst+1);
				if (port_s == NULL)
					return 0;
				dmuci_get_value_by_section_string(port_s, "ethernet_iface_name", &port_name);

			}else if (DM_LSTRNCMP(reference.path, "Device.WiFi.SSID.", 17) == 0){
				inst = strrchr(reference.path, '.');
				port_s = get_dup_section_in_dmmap_opt("dmmap_wireless", "ssid", "ssid_instance", inst+1);
				if (port_s == NULL)
					return 0;
				dmuci_get_value_by_section_string(port_s, "ap_section_name", &port_name);
			}

			// Update port option in dmmap
			dmuci_set_value_by_section(args->bridge_port_dmmap_sec, "port", port_name);

			if (DM_STRCMP(enable, "1") == 0) {
				if (DM_LSTRNCMP(reference.path, "Device.WiFi.SSID.", 17) == 0)
					add_wireless_port_to_bridge_sections(args->bridge_sec, args->bridge_dmmap_sec, port_name);
				else
			...
			// Update Bridge VLANPort instance if exists
			Update_BridgeVLANPort_Port_Layer(refparam, args->bridge_sec, args->bridge_dmmap_sec, args->br_inst, port_name);
			return 0;
		}
	return 0;
}
...
static int br_get_sysfs(const struct bridge_port_args *br, const char *name, char **value)
{
	...
	dmuci_get_value_by_section_string(br->bridge_port_dmmap_sec, "port", &device);
...
static int set_BridgingBridgeVLAN_VLANID(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	switch (action) {
		...
		case VALUESET:
			...
			get_bridge_vlan_config_section(((struct bridge_vlan_args *)data)->bridge_vlan_sec, &br_vlan_dmmap_s);
			...
			dmuci_set_value_by_section(br_vlan_dmmap_s, "vlan", value);
...
static int get_BridgingBridgeVLANPort_VLAN(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "VLAN_path", value);
	...
	if ((*value)[0] == '\0') {
		...
		dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "vid", &vlan);

		adm_entry_get_reference_param(ctx, br_vlan_path, vlan, value);

		// Store Port value
		dmuci_set_value_by_section(args->bridge_vlanport_dmmap_sec, "VLAN_path", *value);
	} else {
		if (!adm_entry_object_exists(ctx, *value))
			*value = "";
	}
...
{
	struct bridge_vlanport_args *args = (struct bridge_vlanport_args *)data;
	char VLAN_path[256] = {0};
	char *vlan_inst = NULL, *vid = NULL, *port = NULL;
	struct dm_reference reference = {0};

	bbf_get_reference_args(value, &reference);

	snprintf(VLAN_path, sizeof(VLAN_path), "Device.Bridging.Bridge.%s.VLAN.", args->br_inst);
	vlan_inst = (DM_STRRCHR(value, '.')+1);
	...
	switch (action) {
		case VALUECHECK:
			...
			if (!strstr(value,VLAN_path))
				return FAULT_9007;

			if (get_section_in_dmmap_with_options_eq("dmmap_bridge_vlan", "bridge_vlan", "br_inst", args->br_inst, "bridge_vlan_instance", vlan_inst) == NULL)
				return FAULT_9007;

			return 0;
		case VALUESET:

			// Get VLAN value under dmmap section
			dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "vid", &vid);
			dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "Port", &port);

			if ((DM_STRLEN(port) != 0) && (DM_STRLEN(vid) !=0 )){
				//get config section
				get_bridge_vlan_config_section(args->bridge_vlanport_dmmap_sec, &(args->bridge_vlanport_sec));
				dmuci_del_list_value_by_section(args->bridge_vlanport_sec, "ports", port);
			}

			struct uci_section *vlan_dmmap_sec = get_section_in_dmmap_with_options_eq("dmmap_bridge_vlan", "bridge_vlan", "br_inst", args->br_inst, "bridge_vlan_instance", vlan_inst);
			dmuci_get_value_by_section_string(vlan_dmmap_sec, "vid", &vid);

			// Store VLAN value under dmmap section
			dmuci_set_value_by_section(args->bridge_vlanport_dmmap_sec, "vid", vid);
			...
			if (args->bridge_vlanport_sec)
				dmuci_add_list_value_by_section(args->bridge_vlanport_sec, "ports", port);
...
static int get_BridgingBridgeVLANPort_Port(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "Port_path", value);
	...
	if ((*value)[0] == '\0') {
		...
		dmuci_get_value_by_section_string(args->bridge_vlanport_dmmap_sec, "Port", &name);
		...
		// Store Port value
		dmuci_set_value_by_section(args->bridge_vlanport_dmmap_sec, "Port_path", *value);
	} 
...
static int set_BridgingBridgeVLANPort_Port(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	char *port_inst = NULL, *port = NULL, *vid = NULL;
	struct dm_reference reference = {0};

	bbf_get_reference_args(value, &reference);

	snprintf(Port_path, sizeof(Port_path), "Device.Bridging.Bridge.%s.Port.", args->br_inst);

	port_inst = (DM_STRRCHR(value, '.')+1);
	...
	switch (action) {
		case VALUECHECK:
			...
			if (!strstr(value,Port_path))
				return FAULT_9007;
			
			if (get_section_in_dmmap_with_options_eq("dmmap_bridge_port", "bridge_port", "br_inst", args->br_inst, "bridge_port_instance", port_inst) == NULL)
				return FAULT_9007;
		...
		case VALUESET:
			...
			if ((DM_STRLEN(port) != 0) && (DM_STRLEN(vid) !=0 )){
				//update bridge_vlanport config sec
				get_bridge_vlan_config_section(args->bridge_vlanport_dmmap_sec, &(args->bridge_vlanport_sec));
				dmuci_del_list_value_by_section(args->bridge_vlanport_sec, "ports", port);
			}

			struct uci_section *br_port = get_section_in_dmmap_with_options_eq("dmmap_bridge_port", "bridge_port", "br_inst", args->br_inst, "bridge_port_instance", port_inst);
			dmuci_get_value_by_section_string(br_port, "port", &port);

			// Store port path under dmmap section
			dmuci_set_value_by_section(args->bridge_vlanport_dmmap_sec, "Port_path", value);
			dmuci_set_value_by_section(args->bridge_vlanport_dmmap_sec, "Port", port);
...
{
	char *port = NULL;
	dmuci_get_value_by_section_string(((struct bridge_vlanport_args *)data)->bridge_vlanport_sec, "ports", &port);
	if (DM_STRLEN(port))
		*value = (DM_STRSTR(port, ":t")) ? "0" : "1";
	return 0;
}
...
static int set_BridgingBridgeVLANPort_Untagged(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		...
		case VALUESET:
			string_to_bool(value, &b);
			dmuci_get_value_by_section_string(((struct bridge_vlanport_args *)data)->bridge_vlanport_dmmap_sec, "Port", &port);
			if (!b){
				snprintf(port_buf, sizeof(port_buf), "%s:t", port);
				dmuci_set_value_by_section(((struct bridge_vlanport_args *)data)->bridge_vlanport_sec, "ports", port_buf);
			}else{
				dmuci_set_value_by_section(((struct bridge_vlanport_args *)data)->bridge_vlanport_sec, "ports", port);
			}
			dmubus_call_set("network", "reload", UBUS_ARGS{0}, 0);
			return 0;
	}
